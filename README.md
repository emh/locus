...
Start the frontend and all three local Workers:
...
For local development, `.dev.vars` files are already present with `MOCK_LLM="true"` so the app can run without OpenAI. To test real extraction locally, set these in `workers/metadata/.dev.vars`:
...
The workflow deploys `locus-metadata`, then `locus-geocode`, then `locus-place`, because the place Worker has service bindings to the other two Workers.
