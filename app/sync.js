...
import { ensureSharedSyncState, loadSettings } from "./storage.js";

const SYNC_PATH = "/sync";
const RETRY_MIN_MS = 1000;
const RETRY_MAX_MS = 15000;

export class LocusSync {
  constructor({ kind, code, state, save, onStatus, onChange, onRoom, onDeleted }) {
    this.kind = kind;
  ...
  handleMessage(raw) {
    ...
    if (message.type === "roomDeleted") {
      this.onDeleted?.(message.code || this.code);
      return;
    }
  ...
  syncState() {
    if (this.kind === "profiles") return this.state.profileSync;
    return ensureSharedSyncState(this.state, this.code);
  }
...
export async function createRemoteProfile({ user, mutations }, settings = loadSettings()) {
...
export async function createRemoteList({ list, user, places, listItems }, settings = loadSettings()) {
  const response = await fetch(getEndpoint(settings.syncBaseUrl, "/api/lists"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ list, user, places, listItems })
  });
...
export async function fetchRemoteList(code, settings = loadSettings()) {
  const response = await fetch(getRoomEndpoint(settings.syncBaseUrl, "lists", code, ""));
  if (!response.ok) throw new Error(await getErrorMessage(response));
  return response.json();
}

export async function joinRemoteList(code, user, settings = loadSettings()) {
  const response = await fetch(getRoomEndpoint(settings.syncBaseUrl, "lists", code, "/collaborators"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
